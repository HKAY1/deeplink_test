...
class IsolateService {
  ...
  Future<T> runIsolateProvider<T>(Future<T> Function() task) async {
    BackgroundIsolateBinaryMessenger.ensureInitialized(
        ServicesBinding.rootIsolateToken!);
    return await Isolate.run<T>(task);
  }

  Future<T> runIsolateProviderWithParams<Q, T>(
    Future<T> Function(Q) task,
    Q message,
  ) async {
    BackgroundIsolateBinaryMessenger.ensureInitialized(
      ServicesBinding.rootIsolateToken!,
    );
    // Wrap the parameterized function in a zero-argument closure.
    return await Isolate.run<T>(() => task(message));
  }
